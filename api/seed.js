...
async function main() {
  ...
  const insertedIssues = await Promise.all(
    Object.keys(rules).map((issueName) =>
      prisma.issue.create({
        data: {
          name: issueName,
        },
  ...
  // Insert symptom names
  const allSymptoms = Object.values(rules).flat();
  const uniqueSymptoms = [...new Set(allSymptoms)];

  const insertedSymptoms = await Promise.all(
    uniqueSymptoms.map((symptomName) =>
  ...
  // Fill in symptoms[] in issue
  await Promise.all(
    Object.entries(rules).map(([issueName, symptoms]) =>
      prisma.issue.update({
        where: { name: issueName },
        data: {
          symptoms: {
            connect: symptoms.map((symptom) => ({ name: symptom })),
          },
        },
      })
    )
  ...
  const adminPassword = await bcrypt.hash("admin", 10); // Hash the password
  ...
  console.log(`Created admin user: ${createdAdmin.username}`);
}
